...
const authCheck = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const allUsers = yield User_1.User.find({ email: { $ne: decoded.email } }, { email: 1, _id: 0 }).lean().then(users => users.map(user => user.email));
        // Filter out users who are already in friend_requests or friend_list
        const availableUsers = allUsers.filter(email => !user.friend_requests.includes(email) &&
            !user.friend_list.includes(email));
        // Filter out users from friend_requests who are already in friend_list
        const filteredFriendRequests = user.friend_requests.filter(email => !user.friend_list.includes(email));
        return res.json({
